...
export function ContrastBadge({ label, result }: ContrastBadgeProps) {
  ...
  return (
    <div
      className={`contrast-badge ${isPass ? "contrast-badge--pass" : "contrast-badge--fail"}`}
    >
      ...
      <span
        className={`contrast-badge__tag contrast-badge__tag--${result.level.toLowerCase().replace(" ", "-")}`}
      >
        {result.level}
      </span>
    </div>
