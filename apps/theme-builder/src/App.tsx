...
export function App() {
  ...
  useEffect(() => {
    ...
    root.style.setProperty('--ui-secondary-foreground', tokens.secondaryForeground);
    ...
    root.style.setProperty('--ui-destructive-foreground', tokens.destructiveForeground);
    ...
    <div className="theme-builder-app">
      <header className="app-header">
        ...
        <div style={{ fontSize: '0.875rem', color: 'var(--app-muted)' }}>
          Active Preset: <strong style={{ color: '#38bdf8' }}>{tokens.name}</strong> ({tokens.mode})
        </div>
      </header>
