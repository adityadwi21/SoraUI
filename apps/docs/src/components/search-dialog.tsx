...
import { Search, BookOpen, Box, Layers, LayoutTemplate, Palette, ArrowRight } from 'lucide-react';
...
export const SearchDialog: React.FC<SearchDialogProps> = ({ open, onClose, onNavigate }) => {
  ...
  const all: SearchResultItem[] = useMemo(() => {
    ...
    TEMPLATE_DOCS.forEach((t) =>
      r.push({ id: `t-${t.id}`, title: t.name, category: 'Template', description: t.description, href: `/templates/${t.id}` })
    );
  ...
  return (
    <div className="docs-search-backdrop" onClick={onClose} role="dialog" aria-modal aria-label="Search">
      <div className="docs-search-panel" onClick={(e) => e.stopPropagation()}>
        {/* Input row */}
        <div className="docs-search-input-row">
          <Search size={18} style={{ color: 'var(--docs-fg-muted)', flexShrink: 0 }} />
          <input
            ref={inputRef}
            type="text"
            ...
            items.map((item, idx) => (
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem', width: '100%' }}>
                  ...
                  <ArrowRight size={14} style={{ opacity: idx === sel ? 0.8 : 0, transition: 'opacity 120ms', flexShrink: 0 }} />
                </div>
              </button>
            ))
          )}
