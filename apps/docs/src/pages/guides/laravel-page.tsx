...
export const LaravelPage: React.FC<LaravelPageProps> = ({ onNavigate }) => {
  ...
  const getCmd = (
    ...
    if (type === "init") {
      switch (tab) {
        case "pnpm":
          return `pnpm dlx @soraui/cli@latest init ${extra}`.trim();
        case "npm":
          return `npx @soraui/cli@latest init ${extra}`.trim();
        case "yarn":
          return `yarn dlx @soraui/cli@latest init ${extra}`.trim();
        case "bun":
          return `bunx --bun @soraui/cli@latest init ${extra}`.trim();
      }
    }
    if (type === "add") {
      switch (tab) {
    ...
    <article className="docs-page sora-intro-manifesto">
      ...
      <header className="docs-intro-header">
        ...
        <p className="docs-intro-lead">
          Install and configure Laravel with Inertia.js React and SoraUI design
          tokens.
        </p>
      </header>
      ...
      <section className="docs-intro-section" id="use-theme-builder">
        ...
        <p>
          Pick a cosmic theme preset, fine-tune the 20 CSS tokens, and generate
          ready-to-use tokens for your Laravel app:
        </p>
      ...
      <section className="docs-intro-section" id="use-the-cli">
        ...
        <p>First create a fresh Laravel application with Inertia.js React:</p>
        <CodeBlock language="bash" code="laravel new my-app --react" />
      ...
      <section className="docs-intro-section" id="existing-project">
        ...
        <div className="docs-tabbed-codeblock">
          ...
          <pre className="docs-tabbed-codeblock-pre">
            <code>
              {tab2 === "pnpm" &&
                "pnpm add @soraui/react @soraui/core lucide-react clsx"}
              {tab2 === "npm" &&
                "npm install @soraui/react @soraui/core lucide-react clsx"}
              {tab2 === "yarn" &&
                "yarn add @soraui/react @soraui/core lucide-react clsx"}
              {tab2 === "bun" &&
                "bun add @soraui/react @soraui/core lucide-react clsx"}
            </code>
          </pre>
        </div>
        ...
        <p>
          Ensure <code>vite.config.js</code> resolves <code>@</code> to{" "}
  ...
  resolve: {
    alias: {
      '@': path.resolve(__dirname, './resources/js'),
        ...
        <h3 className="docs-intro-h3" style={{ marginTop: "1.5rem" }}>
          3. Import SoraUI CSS Tokens
        </h3>
        <p>
          Import the base tokens in <code>resources/css/app.css</code>:
        </p>
        <CodeBlock
          language="css"
