...
test.describe('12E — FOUC Prevention: data-theme applied before React hydration', () => {
  for (const theme of THEMES) {
    test(`Theme "${theme}": data-theme present in initial HTML (no FOUC)`, async ({ page }) => {
    ...
    test(`Theme "${theme}": --ui-background CSS variable resolves before hydration`, async ({ page }) => {
      await page.goto(`/?theme=${theme}`, { waitUntil: 'domcontentloaded' });

      // Evaluate computed CSS custom property value
      const bg = await page.evaluate(() =>
        getComputedStyle(document.documentElement).getPropertyValue('--ui-background').trim()
      );
  ...
  test('ThemeScope: nested theme does not bleed into parent', async ({ page }) => {
    ...
    const parentBg = await page.evaluate(() => {
      ...
      return parent ? getComputedStyle(parent).getPropertyValue('--ui-background').trim() : null;
    });

    // Child (sky) background
    const childBg = await page.evaluate(() => {
      const child = document.querySelector('[data-testid="child-scope"]');
      return child ? getComputedStyle(child).getPropertyValue('--ui-background').trim() : null;
    });
