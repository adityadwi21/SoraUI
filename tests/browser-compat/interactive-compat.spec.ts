...
test.describe("12F — Tooltip cross-browser smoke", () => {
  ...
  test("shows tooltip on hover with correct role", async ({ page }) => {
    const trigger =
      page
        .getByRole("button")
        .filter({ has: page.locator("[data-tooltip-trigger]") })
        .first() ?? page.locator('[data-testid="tooltip-trigger"]').first();
    await trigger.hover();
...
test.describe("12F — ThemeScope cross-browser CSS variable smoke", () => {
  test("CSS custom properties cascade correctly in nested ThemeScope", async ({
    page,
  }) => {
    ...
    const parentBg = await page.evaluate(() => {
      ...
      return el
        ? getComputedStyle(el).getPropertyValue("--ui-background").trim()
        : "";
    });
...
test.describe("12A (browser) — Dialog ARIA structure", () => {
  test("open dialog has role=dialog, aria-modal, aria-labelledby", async ({
    page,
  }) => {
    await page.goto("/components/dialog");
    await page
...
test.describe("12A (browser) — Combobox ARIA structure", () => {
  test("has role=combobox with aria-expanded and aria-haspopup", async ({
    page,
  }) => {
