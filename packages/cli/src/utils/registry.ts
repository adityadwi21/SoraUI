...
export const ALL_COMPONENTS: RegistryItem[] = embeddedRegistry.components.map((c: any) => ({
  name: c.name,
...
export const ALL_BLOCKS: BlockItem[] = embeddedRegistry.blocks.map((b: any) => ({
  id: b.id,
  name: b.name,
  category: b.category,
...
export const ALL_TEMPLATES: TemplateItem[] = embeddedRegistry.templates.map((t: any) => ({
  id: t.id,
  name: t.name,
  category: t.category,
  blocks: t.blocks || [],
...
export const ALL_THEMES: ThemeItem[] = (embeddedRegistry.themes || []).map((th: any) => ({
  id: th.id,
  label: th.label,
  mode: th.mode,
}));
