...
export async function getConfig(cwd: string = process.cwd()): Promise<SoraConfig> {
...
export async function writeConfig(config: SoraConfig, cwd: string = process.cwd()): Promise<void> {
  const configPath = join(cwd, 'soraui.config.json');
