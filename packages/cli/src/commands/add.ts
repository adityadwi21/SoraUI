...
export const addCommand = new Command('add')
  .description('Add components, blocks, or themes to your project from the canonical registry')
  .argument('[target...]', 'Component names (e.g. button, dialog), or "block <id>", or "theme <id>"')
  ...
  .action(async (targets: string[] = [], options) => {
    ...
    if (targets[0]?.toLowerCase() === 'theme') {
      ...
      if (!themeId) {
        console.log(pc.red('  Error: Please specify a theme preset id (e.g. `npx soraui add theme sky`).'));
        console.log(pc.dim(`  Available themes: ${ALL_THEMES.map((t) => t.id).join(', ')}\n`));
        process.exit(1);
      }

      const theme = ALL_THEMES.find((t) => t.id === themeId);
      if (!theme) {
        console.log(pc.red(`  Error: Theme preset "${themeId}" not found in registry.`));
        console.log(pc.dim(`  Available themes: ${ALL_THEMES.map((t) => t.id).join(', ')}\n`));
        process.exit(1);
      }
      ...
      if (existsSync(themePath) && !options.overwrite) {
        console.log(pc.yellow(`  Warning: File styles/sora-theme-${themeId}.css already exists.`));
      ...
      console.log(pc.green('  ✓') + ` Generated styles/sora-theme-${themeId}.css`);
      console.log(pc.dim(`  Import this in your app: import './styles/sora-theme-${themeId}.css';\n`));
      return;
    }
    ...
    if (targets[0]?.toLowerCase() === 'block') {
      ...
      if (!blockId) {
        console.log(pc.red('  Error: Please specify a block id (e.g. `npx soraui add block login-form`).'));
        console.log(pc.dim(`  Available blocks: ${ALL_BLOCKS.map((b) => b.id).join(', ')}\n`));
        process.exit(1);
      }

      const block = ALL_BLOCKS.find((b) => b.id === blockId);
      if (!block) {
        console.log(pc.red(`  Error: Block "${blockId}" not found in registry.`));
        console.log(pc.dim(`  Available blocks: ${ALL_BLOCKS.map((b) => b.id).join(', ')}\n`));
        process.exit(1);
      }
      ...
      console.log(pc.dim(`  Required components: ${requiredComponents.join(', ')}`));
      ...
      console.log(pc.green('  ✓') + ` Generated components/blocks/${blockId}.tsx`);
      ...
      for (const compName of requiredComponents) {
        ...
        if (!existsSync(compPath) || options.overwrite) {
          ...
          console.log(pc.green('  ✓') + ` Generated ${config.componentsPath}/${compName}.tsx`);
        }
      }
    ...
    for (const compName of targets) {
      ...
      if (!componentInfo) {
        console.log(pc.red(`  Error: Component "${name}" not found in registry.`));
        console.log(pc.dim('  Run `npx soraui list` to view all 44 available components.\n'));
        continue;
      }
      ...
      if (existsSync(targetFilePath) && !options.overwrite) {
        console.log(
          pc.yellow(`  Warning: File ${config.componentsPath}/${name}.tsx already exists. Skipping.`)
        );
        continue;
      }
      ...
      console.log(pc.green('  ✓') + ` Generated ${config.componentsPath}/${name}.tsx`);
    }
