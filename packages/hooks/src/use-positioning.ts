...
export function usePositioning(
  ...
  const updatePosition = useCallback(() => {
    ...
    const floatingRect = floatingEl
      ...
      : { width: anchorRect.width, height: 160 };
    ...
    if (flip) {
      if (
        side === "bottom" &&
        anchorRect.bottom + offset + floatingRect.height > viewportHeight
      ) {
        if (anchorRect.top - offset - floatingRect.height >= 0)
          actualSide = "top";
      } else if (
        side === "top" &&
        anchorRect.top - offset - floatingRect.height < 0
      ) {
        if (anchorRect.bottom + offset + floatingRect.height <= viewportHeight)
          actualSide = "bottom";
      } else if (
        side === "right" &&
        anchorRect.right + offset + floatingRect.width > viewportWidth
      ) {
        if (anchorRect.left - offset - floatingRect.width >= 0)
          actualSide = "left";
      } else if (
        side === "left" &&
        anchorRect.left - offset - floatingRect.width < 0
      ) {
        if (anchorRect.right + offset + floatingRect.width <= viewportWidth)
          actualSide = "right";
      }
    }
    ...
    const finalPlacement = (
      align ? `${actualSide}-${align}` : actualSide
    ) as Placement;
