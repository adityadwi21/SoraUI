...
// Layout & Navigation
import { Card, CardHeader, CardTitle, CardContent, CardFooter } from '../../src/components/card/card';
import { Breadcrumb, BreadcrumbList, BreadcrumbItem, BreadcrumbLink, BreadcrumbSeparator, BreadcrumbPage } from '../../src/components/breadcrumb/breadcrumb';
import { Pagination, PaginationContent, PaginationItem, PaginationPrevious, PaginationNext, PaginationLink } from '../../src/components/pagination/pagination';
...
describe('12A — axe-core Accessibility Audit: Level 1 Primitives', () => {
  ...
  it('Button icon — has aria-label, no violations', async () => {
    const { container } = render(<Button size="icon" aria-label="Close menu">✕</Button>);
    await expectNoA11yViolations(container);
  });
  ...
  it('Input with error — no violations', async () => {
    const { container } = render(
      <div>
        ...
        <span id="err-msg" role="alert">Invalid email address</span>
      </div>
    );
    await expectNoA11yViolations(container);
  });
  ...
  it('Slider — no violations', async () => {
    const { container } = render(<Slider aria-label="Volume" min={0} max={100} defaultValue={50} />);
    await expectNoA11yViolations(container);
  });

  it('Statistic — no violations', async () => {
    const { container } = render(<Statistic title="Revenue" value="$12,400" trend="up" trendValue="+12%" />);
    await expectNoA11yViolations(container);
  });
  ...
  it('Collapsible — no violations', async () => {
    const { container } = render(
      <Collapsible>
        ...
        <CollapsibleContent><p>Collapsed content</p></CollapsibleContent>
      </Collapsible>
    );
    await expectNoA11yViolations(container);
  });
...
describe('12A — axe-core Accessibility Audit: Interactive Components (structural)', () => {
  it('Tabs — no violations', async () => {
    const { container } = render(
      <Tabs defaultValue="tab1">
        ...
        <TabsContent value="tab1"><p>Account panel</p></TabsContent>
        <TabsContent value="tab2"><p>Security panel</p></TabsContent>
      </Tabs>
    );
    await expectNoA11yViolations(container);
  });
  ...
  it('FileUploader — no violations', async () => {
    const { container } = render(<FileUploader onFilesChange={() => {}} aria-label="Upload files" />);
    await expectNoA11yViolations(container);
  });
});
