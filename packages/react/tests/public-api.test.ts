...
describe("Phase 10.5A — Public API Snapshot & Identity Verification", () => {
  it("exports exactly all 47 primitives registered in canonical registry.json", () => {
    ...
    for (const comp of registryJson.components) {
      ...
      expect(
        exportName,
        `Missing mapping for registered component "${comp.name}"`,
      ).toBeDefined();
      expect(
        (SoraUI as any)[exportName!],
        `Expected component "${exportName}" to be exported in @soraui/react`,
      ).toBeDefined();
    }
  });
  ...
  it("exports exactly all 14 blocks registered in canonical registry.json", () => {
    ...
    for (const block of registryJson.blocks) {
      ...
      expect(
        exportName,
        `Missing mapping for registered block "${block.id}"`,
      ).toBeDefined();
      expect(
        (SoraUI as any)[exportName!],
        `Expected block "${exportName}" to be exported in @soraui/react`,
      ).toBeDefined();
    }
  });
  ...
  it("exports exactly all 4 page templates registered in canonical registry.json", () => {
    ...
    for (const tpl of registryJson.templates) {
      ...
      expect(
        exportName,
        `Missing mapping for registered template "${tpl.id}"`,
      ).toBeDefined();
      expect(
        (SoraUI as any)[exportName!],
        `Expected template "${exportName}" to be exported in @soraui/react`,
      ).toBeDefined();
    }
  });
