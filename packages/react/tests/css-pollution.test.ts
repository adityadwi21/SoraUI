...
describe('Phase 10.5C — CSS Global Pollution & Namespacing Audit', () => {
  ...
  it('contains 0 bare global element selectors that would pollute consumer apps', () => {
    ...
    const ruleHeaders = cleanCss
      ...
      .filter((chunk) => chunk.length > 0 && !chunk.startsWith('@keyframes') && !chunk.startsWith('@media'));

    // Forbidden bare tag selectors (e.g. "body", "button", "input", "textarea", "select", "*", "a", "p", "h1")
    const forbiddenTags = new Set(['body', 'button', 'input', 'textarea', 'select', '*', 'a', 'p', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'ul', 'ol', 'li']);
    ...
    for (const header of ruleHeaders) {
      ...
      const selectors = lastLine.split(',').map((s) => s.trim()).filter(Boolean);
      for (const sel of selectors) {
        // A bare tag selector is an unadorned HTML tag without . or [ or # or :
        const isBareTag = forbiddenTags.has(sel);
        expect(isBareTag, `Bare tag selector "${sel}" found in CSS header: "${header}"`).toBe(false);
      }
    }
  });
  ...
  it('all component styles are properly namespaced under .sora-*, [data-theme], [data-sora], or :root', () => {
    ...
    const selectorLines = lines
      .map((l) => l.trim())
      .filter((l) => l.endsWith('{') && !l.startsWith('@') && !l.startsWith('from') && !l.startsWith('to') && !l.match(/^\d+%/));
    ...
    for (const line of selectorLines) {
      ...
      for (const sel of selectors) {
        ...
        expect(isAllowed, `Selector "${s}" must be namespaced with .sora- or [data-theme]`).toBe(true);
      }
    }
  });
});
