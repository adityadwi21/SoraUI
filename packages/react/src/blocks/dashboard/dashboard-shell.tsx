  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {sidebarOpen ? (
            <div style={{ fontWeight: 700, fontSize: '1.125rem', color: 'var(--ui-primary, #0ea5e9)' }}>
              {brand}
            </div>
          ) : (
            <div style={{ fontWeight: 700, fontSize: '1.25rem', color: 'var(--ui-primary, #0ea5e9)' }}>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  backgroundColor: isActive ? 'var(--ui-accent, #e0f2fe)' : 'transparent',
                  color: isActive ? 'var(--ui-accent-foreground, #0284c7)' : 'var(--ui-foreground, #0c1a2b)',
                  border: 'none',
                  cursor: 'pointer',
              ...
              >
                {item.icon && <span style={{ fontSize: '1.125rem' }}>{item.icon}</span>}
        ...
        >
          ...
          {sidebarOpen && (
            <div style={{ flex: 1, overflow: 'hidden' }}>
              <div style={{ fontWeight: 600, fontSize: 'var(--sora-text-sm, 0.875rem)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                {user.name}
              </div>
              <div style={{ color: 'var(--ui-muted-foreground, #71717a)', fontSize: 'var(--sora-text-xs, 0.75rem)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                {user.email}
              </div>
            </div>
          )}
            ...
            <Dropdown>
              ...
              <DropdownContent placement="bottom-end">
                <DropdownLabel>
                  ...
                  <div style={{ fontSize: 'var(--sora-text-xs, 0.75rem)', color: 'var(--ui-muted-foreground, #71717a)' }}>{user.email}</div>
                </DropdownLabel>
                <DropdownSeparator />
                ...
                <DropdownSeparator />
                <DropdownItem onClick={onLogout} style={{ color: 'var(--ui-destructive, #ef4444)' }}>
                  Log out
                </DropdownItem>
              </DropdownContent>
            </Dropdown>
