...
describe('Phase 9B — Dashboard Blocks', () => {
  it('DashboardShell renders navigation items and user account details', () => {
    ...
    expect(handleNavigate).toHaveBeenCalledWith(expect.objectContaining({ id: 'analytics' }));
  });
  ...
  it('DataTableBlock handles search and pagination clicks', () => {
    ...
    // Search input
    fireEvent.change(screen.getByPlaceholderText('Search records...'), { target: { value: 'Alice' } });
...
describe('Phase 9B — Marketing Blocks', () => {
  it('HeroSection renders title, badge, and CTA buttons', () => {
    ...
    expect(screen.getByText('Craft sleek, ultra-fast interfaces with zero bloat.')).toBeInTheDocument();
  ...
  it('FeatureGrid renders multiple feature cards', () => {
    render(
      <FeatureGrid
        title="Why SoraUI?"
        features={[
          { id: 'speed', title: 'Ultra Fast', description: 'Zero runtime style generation.' },
          { id: 'a11y', title: 'Accessible', description: 'WAI-ARIA compliant keyboard navigation.' },
        ]}
      />
    );
  ...
  it('PricingTable toggles billing intervals and selects plans', () => {
    ...
    render(
      <PricingTable
        plans={[
          ...
          { id: 'pro', name: 'Pro', price: '$29', period: 'mo', popular: true, features: ['Unlimited Projects', 'Priority Support'] },
        ]}
    ...
    expect(handleSelectPlan).toHaveBeenCalledWith(expect.objectContaining({ id: 'starter' }));
  });
  ...
  it('FAQSection renders accordion questions and expands answers', () => {
    render(
      <FAQSection
        items={[
          { id: 'q1', question: 'Is SoraUI free?', answer: 'Yes, it is MIT licensed.' },
          { id: 'q2', question: 'Does it support Next.js?', answer: 'Yes, both App Router and Pages Router.' },
        ]}
      />
    );
  ...
  it('FooterSection renders columns, links, and newsletter subscription', () => {
    ...
    fireEvent.change(screen.getByPlaceholderText('Enter email'), { target: { value: 'newsletter@example.com' } });
...
describe('Phase 9B — Forms & Settings Blocks', () => {
  it('MultiStepWizard navigates between steps and completes', () => {
    ...
    render(
      <MultiStepWizard
        steps={[
          { id: 'step1', title: 'Account', component: <div>Account Step Content</div> },
          { id: 'step2', title: 'Profile', component: <div>Profile Step Content</div> },
        ]}
  ...
  it('SettingsForm allows editing profile fields and switching preferences', () => {
    ...
    fireEvent.change(screen.getByLabelText(/Full Name/i), { target: { value: 'Jane Architect' } });
    fireEvent.click(screen.getByRole('button', { name: 'Save Changes' }));

    expect(handleSave).toHaveBeenCalledWith(expect.objectContaining({ name: 'Jane Architect' }));
  });
});
  ...
  it('SaaSLandingPageTemplate renders marketing sections', () => {
    render(
      <SaaSLandingPageTemplate
        ...
        features={{ title: 'Features', features: [{ id: 'f1', title: 'Feature 1', description: 'Desc' }] }}
        pricing={{ plans: [{ id: 'p1', name: 'Free', price: '$0', features: ['Core'] }] }}
        faq={{ items: [{ id: 'q1', question: 'Question?', answer: 'Answer.' }] }}
        footer={{ columns: [{ title: 'Links', links: [{ label: 'Home', href: '/' }] }] }}
      />
    );
    ...
    const { container } = render(
      <ThemeScope theme="midnight">
        ...
        <ThemeScope theme="aurora">
          <PricingTable
            plans={[{ id: 'plan1', name: 'Scoped Plan', price: '$99', features: ['Feature A'] }]}
          />
        </ThemeScope>
      </ThemeScope>
    );
  ...
  it('renders all blocks and templates successfully to static HTML via SSR without error', () => {
    const blocksHtml = renderToString(
      <div>
        ...
        <FeatureGrid title="Features" features={[{ id: '1', title: 'F1', description: 'D1' }]} />
        <PricingTable plans={[{ id: '1', name: 'P1', price: '$10', features: ['A'] }]} />
        <FAQSection items={[{ id: '1', question: 'Q', answer: 'A' }]} />
        <FooterSection columns={[{ title: 'Col', links: [{ label: 'L', href: '#' }] }]} />
        <MultiStepWizard steps={[{ id: '1', title: 'S1', component: <div /> }]} />
        <SettingsForm />
        <LoginPageTemplate />
      </div>
    );
