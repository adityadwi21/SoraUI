...
export const ContextMenuTrigger = forwardRef<HTMLDivElement, ContextMenuTriggerProps>(({ onContextMenu, className, ...props }, ref) => {
...
export const ContextMenuContent = forwardRef<HTMLDivElement, ContextMenuContentProps>(({ className, ...props }, ref) => {
  const ctx = useContext(ContextMenuCtx);
  ...
  return (
    <Portal>
      <div
        ...
        role="menu"
        style={{ position: 'fixed', left: `${ctx.position.x}px`, top: `${ctx.position.y}px`, zIndex: 50, ...props.style }}
...
export const ContextMenuItem = forwardRef<HTMLDivElement, ContextMenuItemProps>(({ disabled, className, onClick, onKeyDown, ...props }, ref) => {
  const ctx = useContext(ContextMenuCtx);
  ...
  return (
    <div
      ...
      className={cx('sora-context-menu__item', disabled && 'sora-context-menu__item--disabled', className)}
      {...props}
    />
  );
});
