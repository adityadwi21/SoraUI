...
function ToastItem({
  ...
  return (
    <div
      ...
      className={cx("sora-toast", "sora-toast--" + (data.variant ?? "default"))}
    >
      <div className="sora-toast__body">
        {data.title && <div className="sora-toast__title">{data.title}</div>}
        {data.description && (
          <div className="sora-toast__description">{data.description}</div>
        )}
      </div>
...
>(({ className, children, ...props }, ref) => (
  <div
    ref={ref}
    className={cx("sora-toast__description", className)}
    {...props}
  >
    {children}
  </div>
