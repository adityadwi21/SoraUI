...
>(({ className, children, ...props }, ref) => {
  ...
  return (
    <NavItemContext.Provider
      value={{ open, setOpen, triggerRef: triggerRef as any }}
    >
...
>(({ className, children, ...props }, ref) => {
  ...
  if (!ctx)
    throw new Error("NavigationMenuTrigger must be within NavigationMenuItem");
  ...
  return (
    <button
      ...
      className={cx(
        "sora-nav-menu__trigger",
        ctx.open && "sora-nav-menu__trigger--open",
        className,
      )}
      {...props}
    >
      <span>{children}</span>
      <span className="sora-nav-menu__icon" aria-hidden="true">
        ▾
      </span>
    </button>
  );
...
>(({ className, ...props }, ref) => {
  const ctx = useContext(NavItemContext);
  if (!ctx)
    throw new Error("NavigationMenuContent must be within NavigationMenuItem");
