...
describe('@soraui/core — Token & Theme Engine', () => {
  ...
  it('merges class names with cx utility', () => {
    expect(cx('sora-btn', false, null, 'sora-btn--primary')).toBe('sora-btn sora-btn--primary');
  });
  ...
  describe('validateTheme', () => {
    ...
    it('returns error when required Theme Contract keys are missing', () => {
      ...
      expect(res.errors).toEqual(
        expect.arrayContaining([
          expect.objectContaining({ type: 'missing_key', key: '--ui-background' }),
    ...
    it('returns error when referencing non-existent semantic token', () => {
      ...
      expect(res.errors).toEqual(
        expect.arrayContaining([
          expect.objectContaining({ type: 'invalid_reference', key: '--ui-primary' }),
        ])
      );
    });
    ...
    it('detects and rejects circular token references with full chain', () => {
      ...
      expect(res.valid).toBe(false);
      const circularErr = res.errors.find((e) => e.type === 'circular_reference');
      expect(circularErr).toBeDefined();
      expect(circularErr?.chain).toEqual(['--ui-primary', '--ui-accent', '--ui-secondary', '--ui-primary']);
    });
  ...
  describe('Exporters', () => {
    it('exportThemeToCSS generates valid standalone and default CSS selectors', () => {
      const defaultCss = exportThemeToCSS(validTheme, { themeId: 'sky', isDefault: true });
      ...
      const scopedCss = exportThemeToCSS(validTheme, { themeId: 'midnight', isDefault: false });
      expect(scopedCss).toContain('[data-theme="midnight"] {');
      expect(scopedCss).not.toContain(':root');

      const modeCss = exportThemeToCSS(validTheme, { themeId: 'sky', mode: 'dark' });
  ...
  describe('DOM Theme Scoping & FOUC Script', () => {
    ...
    it('generates non-empty FOUC prevention script', () => {
      const script = getThemeInitScript({ defaultTheme: 'sky', defaultMode: 'system' });
