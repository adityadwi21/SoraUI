...
describe("Phase 11 — 11 MCP Tool Handlers Execution", () => {
  it("1. soraui_get_context returns full ecosystem context", () => {
    ...
    expect(ctx.catalog.totalComponents).toBe(47);
  ...
  it("3. soraui_list lists items accurately", () => {
    ...
    expect(
      listRes.items.every((b) => (b as any).category === "marketing"),
    ).toBe(true);
  });
  ...
  it("8. soraui_compose_recipe produces deterministic JSX with recipeVersion 1.0", () => {
    const recipe = handleComposeRecipe({
      recipe: "auth_flow",
      theme: "midnight",
      mode: "dark",
    });
    ...
    expect(recipe.generatedCode).toContain(
  ...
  it("9. soraui_get_install_commands generates CLI & NPM commands without shell execution", () => {
    const cliRes = handleGetInstallCommands({
      target: "metric-grid",
      type: "block",
      method: "cli",
    });
    ...
    const npmRes = handleGetInstallCommands({
      target: "metric-grid",
      type: "block",
      method: "npm",
    });
  ...
  it("10. soraui_resolve_dependencies computes full dependency graph", () => {
    const deps = handleResolveDependencies({
      id: "login-page",
      kind: "template",
    });
  ...
  it("11. soraui_validate_composition detects violations and outputs diagnostic report", () => {
    const report = handleValidateComposition({
      code: '<div style={{ color: "#ff0000" }}><Button size="icon" /></div>',
    });
    expect(report.valid).toBe(false);
    expect(report.errorsCount).toBeGreaterThan(0);
    expect(report.diagnostics.some((d) => d.rule === "SORA-TOKEN-001")).toBe(
      true,
    );
    expect(report.diagnostics.some((d) => d.rule === "SORA-A11Y-001")).toBe(
      true,
    );
  });
});
