...
describe('Phase 11 — MCP Canonical Registry Sync & Parity', () => {
  it('loads canonical registry with exact 44 primitives, 14 blocks, 4 templates, and 9 themes', () => {
    const registry = loadCanonicalRegistry();

    expect(registry.registryVersion).toBe('0.1.0');
    expect(registry.components.length).toBe(44);
