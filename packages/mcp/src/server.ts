...
export function createMcpServer(): McpServer {
  ...
  server.tool(
    ...
    {
      task: z
        .enum(["build", "debug", "migrate", "compose"])
        .optional()
        .describe("Specific task the agent wants guidance for"),
    },
    async (args) => {
  ...
  server.tool(
    "soraui_search",
    "Search across 47 components, 14 blocks, 4 templates, and 9 themes with deterministic scoring and match reasoning.",
    {
      query: z
        .string()
        .min(1)
        .describe("Search query keyword, component name, or category"),
      kind: z
        .enum(["all", "components", "blocks", "templates", "themes"])
        .optional()
        .describe("Filter by catalog kind"),
      limit: z
        .number()
        .optional()
  ...
  server.tool(
    ...
    {
      kind: z
        .enum(["components", "blocks", "templates", "themes"])
        .describe("Catalog namespace to list"),
      category: z
        .string()
        .optional()
        .describe("Optional category filter (e.g. auth, dashboard, marketing)"),
    },
    async (args) => {
  ...
  server.tool(
    ...
    "Get detailed specification for a SoraUI primitive component including props, WAI-ARIA roles, keyboard keys, CSS tokens, and examples.",
    {
      name: z
        .string()
        .min(1)
        .describe("Component name (e.g. button, dialog, data-table, calendar)"),
    },
    async (args) => {
  ...
  server.tool(
    ...
    "Get block specification, compound slots, dependency list, and UI-Only Boundary Matrix (SoraUI Handles vs Consumer Handles).",
    {
      id: z
        ...
        .min(1)
        .describe(
          "Block ID (e.g. login-form, metric-grid, pricing-table, dashboard-shell)",
        ),
    },
    async (args) => {
  ...
  server.tool(
    ...
    "Get full template specification, composed block hierarchy, and full page layout structure.",
    {
      id: z
        ...
        .min(1)
        .describe(
          "Template ID (e.g. login-page, dashboard-page, saas-landing-page, settings-page)",
        ),
    },
    async (args) => {
  ...
  server.tool(
    ...
    "Get 24-key Theme Contract values, color scales, light/dark mode contrast, and zero-FOUC head init script.",
    {
      id: z
        ...
        .optional()
        .describe(
          "Optional theme preset ID (e.g. sky, midnight, aurora). If omitted, returns all 9 themes overview.",
        ),
    },
    async (args) => {
  ...
  server.tool(
    ...
    {
      recipe: z
        .enum(["auth_flow", "dashboard", "saas_landing", "settings_tabs"])
        .describe("Predefined composition recipe pattern"),
      theme: z
        .string()
        .optional()
        .describe("Theme preset ID (e.g. sky, midnight, aurora)"),
  ...
  server.tool(
    ...
    "Generate exact CLI or NPM shell installation commands for any component, block, or template (read-only command generator).",
    {
      ...
      type: z
        .enum(["component", "block", "template"])
        .optional()
        .describe("Item type"),
      method: z
        .enum(["cli", "npm", "pnpm", "yarn"])
        .optional()
  ...
  server.resource("registry", "soraui://registry", async () => {
  ...
  server.resource("themes", "soraui://themes", async () => {
  ...
  server.resource("guidelines", "soraui://guidelines", async () => {
  ...
  server.prompt(
    "scaffold-page",
    {
      pageType: z
        .string()
        .describe(
          "Type of page to scaffold (e.g. login, dashboard, landing, settings)",
        ),
      theme: z
        .string()
        .optional()
        .describe("Theme preset ID (e.g. midnight, sky, aurora)"),
      requirements: z
        .string()
        .optional()
        .describe("Special page requirements or user features"),
    },
    async (args) => {
  ...
  server.prompt(
    "build-custom-block",
    {
      ...
      category: z
        .string()
        .describe("Category (e.g. analytics, e-commerce, forms)"),
      primitivesToUse: z
        .string()
        .describe(
          "Comma-separated list of SoraUI primitives (e.g. Card, Button, Input)",
        ),
    },
    async (args) => {
