  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    while ((match = HEX_COLOR_REGEX.exec(line)) !== null) {
      ...
      diagnostics.push({
        ...
        snippet: line.trim(),
        suggestion: 'Replace with a CSS token: var(--ui-primary), var(--ui-background), or var(--ui-border).',
      });
    }
    ...
    while ((match = RAW_COLOR_FUNC_REGEX.exec(line)) !== null) {
      ...
      diagnostics.push({
        ...
        column: match.index + 1,
        snippet: line.trim(),
        suggestion: 'Replace with semantic tokens: var(--ui-muted), var(--ui-accent), or var(--ui-destructive).',
      });
    }
  }
