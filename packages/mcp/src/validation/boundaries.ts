...
const FORBIDDEN_BOUNDARY_CALLS = [
  ...
  { pattern: /\b(useQuery|useMutation|useSWR)\s*\(/g, name: 'data fetching hooks' },
  { pattern: /\b(supabase|firebase|prisma|db)\./g, name: 'direct database client' },
];
    ...
    for (const forbidden of FORBIDDEN_BOUNDARY_CALLS) {
      ...
      while ((match = forbidden.pattern.exec(line)) !== null) {
        diagnostics.push({
          ...
          snippet: line.trim(),
          suggestion: 'SoraUI blocks must remain UI-only. Expose an onSubmit/onAction prop callback and handle network requests in your application layer.',
        });
      }
    }
  }
