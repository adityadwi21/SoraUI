...
export function handleGetContext(
  params: { task?: "build" | "debug" | "migrate" | "compose" | undefined } = {},
) {
  ...
  return {
    ...
    description:
      "The lightweight, accessible UI ecosystem for modern web apps.",
    catalog: {
    ...
    rules: {
      noHardcodedColors:
        "Always use var(--ui-*) semantic tokens or theme presets. Never hardcode hex/rgb/hsl colors.",
      noBackendLogicInBlocks:
        "Keep blocks UI-only with onSubmit/onAction callbacks.",
...
function getTaskGuidance(
  task: "build" | "debug" | "migrate" | "compose",
): string {
  switch (task) {
    case "build":
