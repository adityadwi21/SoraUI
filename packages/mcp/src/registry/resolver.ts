import { loadCanonicalRegistry, getBlock, getTemplate, getComponent } from './adapter';
...
export function resolveDependencies(
  ...
  function traverse(currentId: string, currentKind: 'component' | 'block' | 'template') {
    const key = `${currentKind}:${currentId}`;
    if (visited.has(key)) {
      throw new Error(`Circular dependency detected in SoraUI registry graph: ${key}`);
    }
    visited.add(key);
    ...
    if (currentKind === 'template') {
      ...
      if (!template) {
        throw new Error(`Template "${currentId}" not found in canonical registry.`);
      }
      dependencyTree[currentId] = [...template.blocks, ...template.dependencies];
      ...
      if (!block) {
        throw new Error(`Block "${currentId}" not found in canonical registry.`);
      }
      ...
      if (!component) {
        throw new Error(`Component "${currentId}" not found in canonical registry.`);
      }
