...
export function getTheme(id: string): RegistryThemeItem | undefined {
  const registry = loadCanonicalRegistry();
  const lower = id.toLowerCase().trim();
  return registry.themes.find(
    (th) => th.id.toLowerCase() === lower || th.label.toLowerCase() === lower,
  );
}
...
export function searchRegistry(
  ...
  for (const th of registry.themes) {
    checkMatch(
      th.id,
      th.label,
      th.description || `${th.label} (${th.mode}) theme preset`,
      [th.mode],
      th.mode,
      "theme",
    );
